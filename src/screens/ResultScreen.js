...
export default function ResultScreen({ searchQuery: propSearchQuery, imageUri: propImageUri, onBack, route, navigation }) {
  ...
  const getBrowserLogo = (browserId, isActive) => {
    switch (browserId) {
      case 'google':
        return <GoogleIcon isActive={isActive} />;
      case 'bing':
        return <BingIcon isActive={isActive} />;
      case 'yandex':
        return <YandexIcon isActive={isActive} />;
      default:
        return null;
    }
  };
          ...
          return (
            ...
            >
              ...
              <View style={styles.tabContent}>
                {getBrowserLogo(browser.id, isActive)}
              </View>
            </TouchableOpacity>
          );
        })}
